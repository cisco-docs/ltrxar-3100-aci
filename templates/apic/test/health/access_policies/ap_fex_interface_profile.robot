...
{% if fex.expected_state.maximum_critical_faults is defined %}
    Run Keyword If   ${critical} > {{ fex.expected_state.maximum_critical_faults }}   Run Keyword And Continue On Failure
    ...   Fail  "{{ fex_profile_name }} has ${critical} critical faults"
{% endif %}
{% if fex.expected_state.maximum_major_faults is defined %}
    Run Keyword If   ${major} > {{ fex.expected_state.maximum_major_faults }}   Run Keyword And Continue On Failure
    ...   Fail  "{{ fex_profile_name }} has ${major} major faults"
{% endif %}
{% if fex.expected_state.maximum_minor_faults is defined %}
    Run Keyword If   ${minor} > {{ fex.expected_state.maximum_minor_faults }}   Run Keyword And Continue On Failure
