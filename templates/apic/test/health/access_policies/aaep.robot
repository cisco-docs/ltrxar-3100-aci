...
{% if aaep.expected_state.maximum_critical_faults is defined %}
    Run Keyword If   ${critical} > {{ aaep.expected_state.maximum_critical_faults }}   Run Keyword And Continue On Failure
    ...   Fail  "{{ aaep_name }} has ${critical} critical faults"
{% endif %}
{% if aaep.expected_state.maximum_major_faults is defined %}
    Run Keyword If   ${major} > {{ aaep.expected_state.maximum_major_faults }}   Run Keyword And Continue On Failure
    ...   Fail  "{{ aaep_name }} has ${major} major faults"
{% endif %}
{% if aaep.expected_state.maximum_minor_faults is defined %}
    Run Keyword If   ${minor} > {{ aaep.expected_state.maximum_minor_faults }}   Run Keyword And Continue On Failure
