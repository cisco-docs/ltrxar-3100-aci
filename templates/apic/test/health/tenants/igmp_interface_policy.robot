...
{% if igmp_pol.expected_state.maximum_critical_faults is defined %}
    Run Keyword If   ${critical} > {{ igmp_pol.expected_state.maximum_critical_faults }}   Run Keyword And Continue On Failure
    ...   Fail  "{{ igmp_pol_name }} has ${critical} critical faults"
{% endif %}
{% if igmp_pol.expected_state.maximum_major_faults is defined %}
    Run Keyword If   ${major} > {{ igmp_pol.expected_state.maximum_major_faults }}   Run Keyword And Continue On Failure
    ...   Fail  "{{ igmp_pol_name }} has ${major} major faults"
{% endif %}
{% if igmp_pol.expected_state.maximum_minor_faults is defined %}
    Run Keyword If   ${minor} > {{ igmp_pol.expected_state.maximum_minor_faults }}   Run Keyword And Continue On Failure
