...
{% if bd.expected_state.maximum_critical_faults is defined %}
    Run Keyword If   ${critical} > {{ bd.expected_state.maximum_critical_faults }}   Run Keyword And Continue On Failure
    ...   Fail  "{{ bd_name }} has ${critical} critical faults"
{% endif %}
{% if bd.expected_state.maximum_major_faults is defined %}
    Run Keyword If   ${major} > {{ bd.expected_state.maximum_major_faults }}   Run Keyword And Continue On Failure
    ...   Fail  "{{ bd_name }} has ${major} major faults"
{% endif %}
{% if bd.expected_state.maximum_minor_faults is defined %}
    Run Keyword If   ${minor} > {{ bd.expected_state.maximum_minor_faults }}   Run Keyword And Continue On Failure
...
Verify Bridge Domain {{ bd_name }} Health
    ...
    Run Keyword If   ${health} < {{ bd.expected_state.minimum_health }}   Run Keyword And Continue On Failure
...
Verify Bridge Domain {{ bd_name }} Health Post-Check
    ...
    ...   Fail  "{{ bd_name }} health score degraded from ${previous["health"]} to ${health}"
{% endif %}
{% endfor %}
