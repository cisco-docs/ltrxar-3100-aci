...
{% if rl.expected_state.maximum_critical_faults is defined %}
    Run Keyword If   ${critical} > {{ rl.expected_state.maximum_critical_faults }}   Run Keyword And Continue On Failure
    ...   Fail  "{{ rl_name }} has ${critical} critical faults"
{% endif %}
{% if rl.expected_state.maximum_major_faults is defined %}
    Run Keyword If   ${major} > {{ rl.expected_state.maximum_major_faults }}   Run Keyword And Continue On Failure
    ...   Fail  "{{ rl_name }} has ${major} major faults"
{% endif %}
{% if rl.expected_state.maximum_minor_faults is defined %}
    Run Keyword If   ${minor} > {{ rl.expected_state.maximum_minor_faults }}   Run Keyword And Continue On Failure
