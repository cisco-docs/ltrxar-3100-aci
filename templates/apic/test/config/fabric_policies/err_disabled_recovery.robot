...
Verify MCP Loop Policy
    ...
    Should Be Equal Value Json String   ${r.json()}    ${policy}..edrEventP.attributes.recover   {{ 'yes' if apic.fabric_policies.err_disabled_recovery.mcp_loop | default(defaults.apic.fabric_policies.err_disabled_recovery.mcp_loop) else 'no' }}

Verify EP Move Policy
    ${policy}=   Set Variable   $..edrErrDisRecoverPol.children[?(@.edrEventP.attributes.event=='event-ep-move')]
    Should Be Equal Value Json String   ${r.json()}    ${policy}..edrEventP.attributes.recover   {{ 'yes' if apic.fabric_policies.err_disabled_recovery.ep_move | default(defaults.apic.fabric_policies.err_disabled_recovery.ep_move) else 'no' }}

Verify BPDU Guard Policy
    ${policy}=   Set Variable   $..edrErrDisRecoverPol.children[?(@.edrEventP.attributes.event=='event-bpduguard')]
    Should Be Equal Value Json String   ${r.json()}    ${policy}..edrEventP.attributes.recover   {{ 'yes' if apic.fabric_policies.err_disabled_recovery.bpdu_guard | default(defaults.apic.fabric_policies.err_disabled_recovery.bpdu_guard) else 'no' }}
