...
Verify Node Control {{ policy_name }} Policy
    ...
    Should Be Equal JMESPath Json   ${r}    imdata[0].fabricNodeControl.attributes.name   {{ policy_name }}
...
{% if policy.dom | default(defaults.apic.fabric_policies.switch_policies.node_control_policies.dom) %}{% set dom = "Dom" %}{% endif %}
    Should Be Equal JMESPath Json   ${r}    imdata[0].fabricNodeControl.attributes.control   {{ dom }}

Verify Feature Selection for {{ policy_name }}
    Should Be Equal JMESPath Json   ${r}    imdata[0].fabricNodeControl.attributes.featureSel   {{ policy.telemetry | default(defaults.apic.fabric_policies.switch_policies.node_control_policies.telemetry) }}
