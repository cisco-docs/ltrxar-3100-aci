...
Verify Login Domain {{ login_domain.name }}
    ...
    Should Be Equal JMESPath Json   ${r}    imdata[0].aaaLoginDomain.attributes.descr   {{ login_domain.description | default() }}
    Should Be Equal JMESPath Json   ${r}    imdata[0].aaaLoginDomain.attributes.name   {{ login_domain.name }}
    Should Be Equal JMESPath Json   ${r}    imdata[0].aaaLoginDomain.children[?aaaDomainAuth] | [0].aaaDomainAuth.attributes.realm  {{ login_domain.realm }}
...
Verify Login Domain {{ login_domain.name }} TACACS Provider {{ prov.hostname_ip }}
    ...
    Set Suite Variable   $r   ${r.json()}
    Should Be Equal JMESPath Json   ${r}    imdata[0].aaaProviderRef.attributes.name   {{ prov.hostname_ip }}
    Should Be Equal JMESPath Json   ${r}    imdata[0].aaaProviderRef.attributes.order   {{ prov.priority | default(defaults.apic.fabric_policies.aaa.login_domains.tacacs_providers.priority) }}
...
Verify Login Domain {{ login_domain.name }} RADIUS Provider {{ prov.hostname_ip }}
    ...
    Set Suite Variable   $r   ${r.json()}
    Should Be Equal JMESPath Json   ${r}    imdata[0].aaaProviderRef.attributes.name   {{ prov.hostname_ip }}
    Should Be Equal JMESPath Json   ${r}    imdata[0].aaaProviderRef.attributes.order   {{ prov.priority | default(defaults.apic.fabric_policies.aaa.login_domains.radius_providers.priority) }}
...
Verify Login Domain {{ login_domain.name }} LDAP Provider Group
    ...
    Set Suite Variable   $r   ${r.json()}
    Should Be Equal JMESPath Json   ${r}    imdata[0].aaaLdapProviderGroup.attributes.authChoice   {{ login_domain.auth_choice | default(defaults.apic.fabric_policies.aaa.login_domains.auth_choice) }}
