...
Verify SNMP Trap Policy {{ policy_name }}
    ...
    Should Be Equal Value Json String   ${r}    $..snmpGroup.attributes.name   {{ policy_name }}
    Should Be Equal Value Json String   ${r}    $..snmpGroup.attributes.descr   {{ snmp.description | default() }}
...
Verify SNMP Trap Policy {{ policy_name }} Destination {{ dest.hostname_ip }}
    ...
    Should Be Equal Value Json String   ${r}    ${dest}..snmpTrapDest.attributes.host   {{ dest.hostname_ip }}
    Should Be Equal Value Json String   ${r}    ${dest}..snmpTrapDest.attributes.port   {{ dest.port | default(defaults.apic.fabric_policies.monitoring.snmp_traps.destinations.port) }}
    Should Be Equal Value Json String   ${r}    ${dest}..snmpTrapDest.attributes.secName   {{ dest.community }}
    Should Be Equal Value Json String   ${r}    ${dest}..snmpTrapDest.attributes.v3SecLvl   {{ dest.security | default(defaults.apic.fabric_policies.monitoring.snmp_traps.destinations.security) }}
    Should Be Equal Value Json String   ${r}    ${dest}..snmpTrapDest.attributes.ver   {{ dest.version | default(defaults.apic.fabric_policies.monitoring.snmp_traps.destinations.version) }}
