...
Verify AAA Password Profile
    ...
    Should Be Equal Value Json String   ${r.json()}    $..aaaPwdProfile.attributes.changeDuringInterval   {{ 'enable' if apic.fabric_policies.aaa.management_settings.password_change_during_interval | default(defaults.apic.fabric_policies.aaa.management_settings.password_change_during_interval) else 'disable' }}
...
Verify AAA Web Token Settings
    ...
    Should Be Equal Value Json String   ${r.json()}    $..pkiWebTokenData.attributes.sessionRecordFlags   {{ 'login,logout,refresh' if apic.fabric_policies.aaa.management_settings.include_refresh_session_records | default(defaults.apic.fabric_policies.aaa.management_settings.include_refresh_session_records) else 'login,logout' }}

Verify AAA Block Login Profile
    ${r}=   GET On Session   apic   /api/node/mo/uni/userext/blockloginp.json
    Should Be Equal Value Json String   ${r.json()}    $..aaaBlockLoginProfile.attributes.enableLoginBlock   {{ 'enable' if apic.fabric_policies.aaa.management_settings.enable_login_block | default(defaults.apic.fabric_policies.aaa.management_settings.enable_login_block) else 'disable' }}
