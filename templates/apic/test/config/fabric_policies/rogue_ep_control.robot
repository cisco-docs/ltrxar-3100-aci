...
Verify Rogue EP Control
    ...
    Should Be Equal Value Json String   ${r.json()}    $..epControlP.attributes.adminSt   {{ 'enabled' if apic.fabric_policies.rogue_ep_control.admin_state | default(defaults.apic.fabric_policies.rogue_ep_control.admin_state) else 'disabled' }}
