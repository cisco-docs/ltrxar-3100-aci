...
Verify SPAN Source Group {{ span_name }}
    ...
    Should Be Equal Value Json String   ${r}    $..spanSrcGrp.attributes.adminSt   {{ 'enabled' if span.admin_state | default(defaults.apic.fabric_policies.span.source_groups.admin_state) else 'disabled' }}
...
Verify SPAN Source Group {{ span_name }} Source {{ source_name }}
    ...
    Should Be Equal Value Json String   ${r}    ${source}.attributes.dir   {{ source.direction | default(defaults.apic.fabric_policies.span.source_groups.sources.direction) }}
    Should Be Equal Value Json String   ${r}    ${source}.attributes.spanOnDrop   {{ 'yes' if source.span_drop | default(defaults.apic.fabric_policies.span.source_groups.sources.span_drop) else 'no' }}
{% if source.tenant is defined and source.vrf is defined %}
{% set vrf_name = source.vrf ~ defaults.apic.tenants.vrfs.name_suffix %}
    Should Be Equal Value Json String   ${r}    ${source}..spanRsSrcToCtx.attributes.tDn   uni/tn-{{ source.tenant }}/ctx-{{ vrf_name }}
{% endif %}
...
{% set bd_name = source.bridge_domain ~ defaults.apic.tenants.bridge_domains.name_suffix %}
    Should Be Equal Value Json String   ${r}    ${source}..spanRsSrcToBD.attributes.tDn   uni/tn-{{ source.tenant }}/BD-{{ bd_name }}
{% endif %}
...
{% set pod = path.pod_id | default(((apic.node_policies | default()) | community.general.json_query(query))[0] | default('1')) %}
    ...
    Should Be Equal Value Json String   ${r}    ${path}.attributes.tDn   topology/pod-{{ pod }}/paths-{{ path.node_id }}/pathep-[eth{{ path.module | default(defaults.apic.fabric_policies.span.source_groups.sources.access_paths.module) }}/{{ path.port }}]
...
{% set destination_name = span.destination.name ~ defaults.apic.fabric_policies.span.destination_groups.name_suffix %}
    ...
    Should Be Equal Value Json String   ${r}    $..spanSrcGrp.children..spanSpanLbl.attributes.nameAlias   {{ span.destination.alias | default() }}
{% endfor %}
