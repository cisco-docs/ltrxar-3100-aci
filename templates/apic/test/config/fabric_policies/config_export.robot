...
Verify Config Export {{ policy_name }}
    ...
    Should Be Equal Value Json String   ${r.json()}    $..configExportP.attributes.snapshot   {{ 'yes' if policy.snapshot | default(defaults.apic.fabric_policies.config_exports.snapshot) else 'no' }}
