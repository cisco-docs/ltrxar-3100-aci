...
Verify Port Tracking
    ...
    Should Be Equal Value Json String   ${r.json()}    $..infraPortTrackPol.attributes.adminSt   {{ 'on' if apic.fabric_policies.port_tracking.admin_state | default(defaults.apic.fabric_policies.port_tracking.admin_state) else 'off' }}
