...
Verify Spine Switch Policy Group {{ policy_group_name }}
    ...
    Should Be Equal JMESPath Json   ${r}    imdata[0].fabricSpNodePGrp.attributes.name   {{ policy_group_name }}
...
{% set psu_policy_name = pg.psu_policy ~ defaults.apic.fabric_policies.switch_policies.psu_policies.name_suffix %}
    Should Be Equal JMESPath Json   ${r}    imdata[0].fabricSpNodePGrp.children[?fabricRsPsuInstPol] | [0].fabricRsPsuInstPol.attributes.tnPsuInstPolName   {{ psu_policy_name }}
{% endif %}
...
{% set node_control_policy_name = pg.node_control_policy ~ defaults.apic.fabric_policies.switch_policies.node_control_policies.name_suffix %}
    Should Be Equal JMESPath Json   ${r}    imdata[0].fabricSpNodePGrp.children[?fabricRsNodeCtrl] | [0].fabricRsNodeCtrl.attributes.tnFabricNodeControlName   {{ node_control_policy_name }}
{% endif %}
