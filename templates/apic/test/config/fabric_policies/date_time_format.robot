...
Verify Date and Time Format
    ...
    Should Be Equal Value Json String   ${r.json()}    $..datetimeFormat.attributes.showOffset   {{ 'enabled' if apic.fabric_policies.date_time_format.show_offset| default(defaults.apic.fabric_policies.date_time_format.show_offset) else 'disabled' }}  