...
Verify External Connectivity Policy
    ...
    Should Be Equal Value Json String   ${r}    $..fvFabricExtConnP.attributes.name   {{ policy_name }}
    Should Be Equal Value Json String   ${r}    $..fvFabricExtConnP.attributes.rt   {{ apic.fabric_policies.external_connectivity_policy.route_target | default(defaults.apic.fabric_policies.external_connectivity_policy.route_target) }}
    Should Be Equal Value Json String   ${r}    $..fvFabricExtConnP.attributes.id   {{ apic.fabric_policies.external_connectivity_policy.fabric_id | default(defaults.apic.fabric_policies.external_connectivity_policy.fabric_id) }}
    Should Be Equal Value Json String   ${r}    $..fvFabricExtConnP.attributes.siteId   {{ apic.fabric_policies.external_connectivity_policy.site_id | default(defaults.apic.fabric_policies.external_connectivity_policy.site_id) }}
    ...
    {% endif %}
    Should Be Equal Value Json String   ${r}    $..fvPeeringP.attributes.type   {{ type }}
...
Verify External Connectivity Policy Routing Profile {{ routing_profile_name }}
    ...
    Should Be Equal Value Json String   ${r}    ${profile}.attributes.name   {{ routing_profile_name }}
...
Verify External Connectivity Policy Routing Profile {{ routing_profile_name }} Subnet {{ subnet }}
    ...
    Should Be Equal Value Json String   ${r}    ${subnet}.attributes.ip   {{ subnet }}
...
Verify External Connectivity Policy Pod {{ pod.id }}
    ...
    Should Be Equal Value Json String   ${r}    ${pod}..fvPodConnP.attributes.id   {{ pod.id }}
    Should Be Equal Value Json String   ${r}    ${pod}..fvIp.attributes.addr   {{ pod.data_plane_tep }}
