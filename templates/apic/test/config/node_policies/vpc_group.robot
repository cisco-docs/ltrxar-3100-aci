...
Verify VPC Group Mode
    ...
    Should Be Equal Value Json String   ${r}    $..fabricProtPol.attributes.pairT   {{ apic.node_policies.vpc_groups.mode | default(defaults.apic.node_policies.vpc_groups.mode) }}
...
Verify vPC Group {{ vpc_group_name }}
    ...
    Set Suite Variable   $r   ${r.json()}
    Should Be Equal Value Json String   ${r}    $..fabricExplicitGEp.attributes.id   {{ group.id }}
    Should Be Equal Value Json String   ${r}    $..fabricExplicitGEp.attributes.name   {{ group.name | default(vpc_group_name) }}
{% if group.policy is defined %}
    Should Be Equal Value Json String   ${r}    $..fabricRsVpcInstPol.attributes.tnVpcInstPolName   {{ vpc_policy_name }}
{% endif %}
    ${list} =   Create List   {{ group.switch_1 }}   {{ group.switch_2 }}
    Should Be Equal Value Json List   ${r}    $..fabricExplicitGEp.children[*].fabricNodePEp.attributes.id   ${list}
