...
Verify VPC Group Mode
    ...
    Should Be Equal JMESPath Json   ${r}    imdata[0].fabricProtPol.attributes.pairT   {{ apic.node_policies.vpc_groups.mode | default(defaults.apic.node_policies.vpc_groups.mode) }}
...
Verify vPC Group {{ vpc_group_name }}
    ...
    Set Suite Variable   $r   ${r.json()}
    Should Be Equal JMESPath Json   ${r}    imdata[0].fabricExplicitGEp.attributes.id   {{ group.id }}
    Should Be Equal JMESPath Json   ${r}    imdata[0].fabricExplicitGEp.attributes.name   {{ group.name | default(vpc_group_name) }}
{% if group.policy is defined %}
    Should Be Equal JMESPath Json   ${r}    imdata[0].fabricExplicitGEp.children[?fabricRsVpcInstPol] | [0].fabricRsVpcInstPol.attributes.tnVpcInstPolName   {{ vpc_policy_name }}
{% endif %}
    ${list} =   Create List   {{ group.switch_1 }}   {{ group.switch_2 }}
    Should Be Equal JMESPath Json List   ${r}    imdata[0].fabricExplicitGEp.children[*].fabricNodePEp.attributes.id   ${list}
