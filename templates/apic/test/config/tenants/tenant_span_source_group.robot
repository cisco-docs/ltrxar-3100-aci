...
Verify Tenant SPAN Source Group {{ span_grp_name }}
    ...
    Should Be Equal Value Json String   ${r}   $..spanSrcGrp.attributes.name   {{ span_grp_name }}
    Should Be Equal Value Json String   ${r}   $..spanSrcGrp.attributes.descr   {{ span.description | default() }}
    Should Be Equal Value Json String   ${r}   $..spanSrcGrp.attributes.adminSt   {{ 'enabled' if span.admin_state | default(defaults.apic.tenants.policies.span.source_groups.admin_state) else 'disabled' }}
    Should Be Equal Value Json String   ${r}   $..spanSpanLbl.attributes.name   {{ span_destination_name }}
...
Verify Tenant SPAN Source Group {{ span_grp_name }} Source {{ source.name }}
    ...
    Should Be Equal Value Json String   ${r}   ${src}.attributes.name   {{ source.name  }}
    Should Be Equal Value Json String   ${r}   ${src}.attributes.descr   {{ source.description | default()}}
    Should Be Equal Value Json String   ${r}   ${src}.attributes.dir   {{ source.direction | default(defaults.apic.tenants.policies.span.source_groups.sources.direction ) }}
...
{% set endpoint_group_name = source.endpoint_group ~ defaults.apic.tenants.application_profiles.endpoint_groups.name_suffix %}
    Should Be Equal Value Json String   ${r}   ${src}..spanRsSrcToEpg.attributes.tDn   uni/tn-{{ tenant.name }}/ap-{{ application_profile_name }}/epg-{{ endpoint_group_name }}
{% endif %}
