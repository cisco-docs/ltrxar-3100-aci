...
Verify Device Selection Policy Contract {{ contract_name }} Service Graph Template {{ sgt_name }}
    ...
    Should Be Equal Value Json String   ${r}   $..vnsLDevCtx.attributes.ctrctNameOrLbl   {{ contract_name }}
    Should Be Equal Value Json String   ${r}   $..vnsLDevCtx.attributes.graphNameOrLbl   {{ sgt_name }}
    Should Be Equal Value Json String   ${r}   $..vnsLDevCtx.attributes.nodeNameOrLbl   {{ dsp.node_name | default("N1") }}
    Should Be Equal Value Json String   ${r}   $..vnsRsLDevCtxToLDev.attributes.tDn   uni/tn-{{ sgt.device.tenant | default(tenant.name) }}/lDevVip-{{ dev_name }}
...
Verify Device Selection Policy Contract {{ contract_name }} Service Graph Template {{ sgt_name }} Consumer
    ...
    Should Be Equal Value Json String   ${r}   ${consumer}..vnsLIfCtx.attributes.l3Dest   {{ 'yes' if dsp.consumer.l3_destination | default(defaults.apic.tenants.services.device_selection_policies.consumer.l3_destination) else 'no'}}
    Should Be Equal Value Json String   ${r}   ${consumer}..vnsLIfCtx.attributes.permitLog   {{ 'yes' if dsp.consumer.permit_logging | default(defaults.apic.tenants.services.device_selection_policies.consumer.permit_logging) else 'no'}}
...
{% set pol_name = dsp.consumer.redirect_policy.name ~ defaults.apic.tenants.services.redirect_policies.name_suffix %}
    Should Be Equal Value Json String   ${r}   ${consumer}..vnsRsLIfCtxToSvcRedirectPol.attributes.tDn   uni/tn-{{ dsp.consumer.redirect_policy.tenant | default(tenant.name) }}/svcCont/svcRedirectPol-{{ pol_name }}
{% endif %}
...
{% set bd_name = dsp.consumer.bridge_domain.name ~ defaults.apic.tenants.bridge_domains.name_suffix %}
    Should Be Equal Value Json String   ${r}   ${consumer}..vnsRsLIfCtxToBD.attributes.tDn   uni/tn-{{ dsp.consumer.bridge_domain.tenant | default(tenant.name) }}/BD-{{ bd_name }}
...
{% set eepg_name = dsp.consumer.external_endpoint_group.name ~ defaults.apic.tenants.l3outs.external_endpoint_groups.name_suffix %}
    Should Be Equal Value Json String   ${r}   ${consumer}..vnsRsLIfCtxToInstP.attributes.redistribute   {{ redistribute | join(',') }}
    Should Be Equal Value Json String   ${r}   ${consumer}..vnsRsLIfCtxToInstP.attributes.tDn   uni/tn-{{ dsp.consumer.external_endpoint_group.tenant | default(tenant.name) }}/out-{{ l3out_name }}/instP-{{ eepg_name }}
{% set int_name = dsp.consumer.logical_interface ~ defaults.apic.tenants.services.l4l7_devices.logical_interfaces.name_suffix %}
    Should Be Equal Value Json String   ${r}   ${consumer}..vnsRsLIfCtxToLIf.attributes.tDn   uni/tn-{{ sgt.device.tenant | default(tenant.name) }}/lDevVip-{{ dev_name }}/lIf-{{ int_name }}
{% endif %}
...
{% set pol_name = dsp.consumer.service_epg_policy ~ defaults.apic.tenants.services.service_epg_policies.name_suffix %}
    Should Be Equal Value Json String   ${r}   ${consumer}..vnsRsLIfCtxToSvcEPgPol.attributes.tDn   uni/tn-{{ tenant.name }}/svcCont/svcEPgPol-{{ pol_name }}
{% endif %}
...
{% set custom_qos_policy_name = dsp.consumer.custom_qos_policy ~ defaults.apic.tenants.policies.custom_qos.name_suffix %}
    Should Be Equal Value Json String   ${r}   ${consumer}..vnsRsLIfCtxToCustQosPol.attributes.tnQosCustomPolName   {{ custom_qos_policy_name }}
{% endif %}
...
Verify Device Selection Policy Contract {{ contract_name }} Service Graph Template {{ sgt_name }} Provider
    ...
    Should Be Equal Value Json String   ${r}   ${provider}..vnsLIfCtx.attributes.l3Dest   {{ 'yes' if dsp.provider.l3_destination | default(defaults.apic.tenants.services.device_selection_policies.provider.l3_destination) else 'no'}}
    Should Be Equal Value Json String   ${r}   ${provider}..vnsLIfCtx.attributes.permitLog   {{ 'yes' if dsp.provider.permit_logging | default(defaults.apic.tenants.services.device_selection_policies.provider.permit_logging) else 'no'}}
...
{% set pol_name = dsp.provider.redirect_policy.name ~ defaults.apic.tenants.services.redirect_policies.name_suffix %}
    Should Be Equal Value Json String   ${r}   ${provider}..vnsRsLIfCtxToSvcRedirectPol.attributes.tDn   uni/tn-{{ dsp.provider.redirect_policy.tenant | default(tenant.name) }}/svcCont/svcRedirectPol-{{ pol_name }}
{% endif %}
...
{% set bd_name = dsp.provider.bridge_domain.name ~ defaults.apic.tenants.bridge_domains.name_suffix %}
    Should Be Equal Value Json String   ${r}   ${provider}..vnsRsLIfCtxToBD.attributes.tDn   uni/tn-{{ dsp.provider.bridge_domain.tenant | default(tenant.name) }}/BD-{{ bd_name }}
...
{% set eepg_name = dsp.provider.external_endpoint_group.name ~ defaults.apic.tenants.l3outs.external_endpoint_groups.name_suffix %}
    Should Be Equal Value Json String   ${r}   ${provider}..vnsRsLIfCtxToInstP.attributes.redistribute   {{ redistribute | join(',') }}
    Should Be Equal Value Json String   ${r}   ${provider}..vnsRsLIfCtxToInstP.attributes.tDn   uni/tn-{{ dsp.provider.external_endpoint_group.tenant | default(tenant.name) }}/out-{{ l3out_name }}/instP-{{ eepg_name }}
{% set int_name = dsp.provider.logical_interface ~ defaults.apic.tenants.services.l4l7_devices.logical_interfaces.name_suffix %}
    Should Be Equal Value Json String   ${r}   ${provider}..vnsRsLIfCtxToLIf.attributes.tDn   uni/tn-{{ sgt.device.tenant | default(tenant.name) }}/lDevVip-{{ dev_name }}/lIf-{{ int_name }}
{% endif %}
...
{% set pol_name = dsp.provider.service_epg_policy ~ defaults.apic.tenants.services.service_epg_policies.name_suffix %}
    Should Be Equal Value Json String   ${r}   ${provider}..vnsRsLIfCtxToSvcEPgPol.attributes.tDn   uni/tn-{{ tenant.name }}/svcCont/svcEPgPol-{{ pol_name }}
{% endif %}
...
{% set custom_qos_policy_name = dsp.provider.custom_qos_policy ~ defaults.apic.tenants.policies.custom_qos.name_suffix %}
    Should Be Equal Value Json String   ${r}   ${provider}..vnsRsLIfCtxToCustQosPol.attributes.tnQosCustomPolName   {{ custom_qos_policy_name }}
{% endif %}
...
Verify Device Selection Policy Contract {{ contract_name }} Service Graph Template {{ sgt_name }} Copy
    ...
    Should Be Equal Value Json String   ${r}   ${copy}..vnsLIfCtx.attributes.l3Dest   {{ 'yes' if dsp.copy_service.l3_destination | default(defaults.apic.tenants.services.device_selection_policies.copy_service.l3_destination) else 'no'}}
    Should Be Equal Value Json String   ${r}   ${copy}..vnsLIfCtx.attributes.permitLog   {{ 'yes' if dsp.copy_service.permit_logging | default(defaults.apic.tenants.services.device_selection_policies.copy_service.permit_logging) else 'no'}}
...
{% set pol_name = dsp.copy_service.service_epg_policy ~ defaults.apic.tenants.services.service_epg_policies.name_suffix %}
    Should Be Equal Value Json String   ${r}   ${copy}..vnsRsLIfCtxToSvcEPgPol.attributes.tDn   uni/tn-{{ tenant.name }}/svcCont/svcEPgPol-{{ pol_name }}
{% endif %}
{% if dsp.provider.custom_qos_policy is defined %}
{% set custom_qos_policy_name = dsp.copy_service.custom_qos_policy ~ defaults.apic.tenants.policies.custom_qos.name_suffix %}
    Should Be Equal Value Json String   ${r}   ${copy}..vnsRsLIfCtxToCustQosPol.attributes.tnQosCustomPolName   {{ custom_qos_policy_name }}
{% endif %}
{% set int_name = dsp.copy_service.logical_interface ~ defaults.apic.tenants.services.l4l7_devices.logical_interfaces.name_suffix %}
    Should Be Equal Value Json String   ${r}   ${copy}..vnsRsLIfCtxToLIf.attributes.tDn   uni/tn-{{ sgt.device.tenant | default(tenant.name) }}/lDevVip-{{ dev_name }}/lIf-{{ int_name }}
{% endif %}
