...
Verify BFD Interface Policy {{ bfd_name }}
    ...
    Should Be Equal Value Json String   ${r.json()}   $..bfdIfPol.attributes.ctrl   {{ 'opt-subif' if bfd.subinterface_optimization | default(defaults.apic.tenants.policies.bfd_interface_policies.subinterface_optimization) }}
    ...
    Should Be Equal Value Json String   ${r.json()}   $..bfdIfPol.attributes.echoAdminSt   {{ 'enabled' if bfd.echo_admin_state | default(defaults.apic.tenants.policies.bfd_interface_policies.echo_admin_state) else 'disabled' }}
