...
Verify OSPF Timer Policy {{ policy_name }}
    ...
    Should Be Equal Value Json String   ${r.json()}   $..ospfCtxPol.attributes.grCtrl   {{ 'helper' if otp.graceful_restart | default(defaults.apic.tenants.policies.ospf_timer_policies.graceful_restart) else '' }}
