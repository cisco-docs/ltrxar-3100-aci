...
Verify Netflow Monitor {{ monitor_name }}
    ...
    Should Be Equal JMESPath Json   ${r.json()}    imdata[0].netflowMonitorPol.attributes.name   {{ monitor_name }}
    Should Be Equal JMESPath Json   ${r.json()}    imdata[0].netflowMonitorPol.attributes.descr   {{ monitor.description | default() }}
...
Verify Netflow Monitor {{ monitor_name }} Record {{ record_name }}
    Should Be Equal JMESPath Json   ${r.json()}    imdata[0].netflowMonitorPol.children[?netflowRsMonitorToRecord] | [0].netflowRsMonitorToRecord.attributes.tnNetflowRecordPolName   {{ record_name }}
{% endif %}
...
Verify Netflow Monitor {{ monitor_name }} Exporter {{ exporter_name }}
    ...
    Should Be Equal JMESPath Json   ${r.json()}    ${exp}.attributes.tnNetflowExporterPolName   {{ exporter_name }}
{% endfor %}
