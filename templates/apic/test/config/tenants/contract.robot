...
Verify Contract {{ contract_name }} Subject {{ subject_name }}
    ...
    Should Be Equal Value Json String   ${r.json()}   ${subject}..vzSubj.attributes.revFltPorts   {{ ('yes' if (subject.reverse_filter_ports | default(defaults.apic.tenants.contracts.subjects.reverse_filter_ports)) else 'no') if subject.filters is defined  else 'no' }}
...
{% if subject.service_graph is defined and subject.filters is defined %}
    Should Be Equal Value Json String   ${r.json()}   ${subject}..vzRsSubjGraphAtt.attributes.tnVnsAbsGraphName   {{ subject.service_graph }}
{% endif %}
...
Verify Contract {{ contract_name }} Subject {{ subject_name }} Consumer_to_Provider
    ...
    Should Be Equal Value Json String   ${r.json()}   ${consumer_to_provider}..attributes.prio   {{ subject.consumer_to_provider.qos_class | default(defaults.apic.tenants.contracts.subjects.consumer_to_provider.qos_class)}}
    Should Be Equal Value Json String   ${r.json()}   ${consumer_to_provider}..attributes.targetDscp   {{ subject.consumer_to_provider.target_dscp | default(defaults.apic.tenants.contracts.subjects.consumer_to_provider.target_dscp) }}
 {% if subject.consumer_to_provider.service_graph is defined and subject.consumer_to_provider.filters is defined %}
    Should Be Equal Value Json String   ${r.json()}   ${consumer_to_provider}..vzRsInTermGraphAtt.attributes.tnVnsAbsGraphName   {{ subject.consumer_to_provider.service_graph | default(defaults.apic.tenants.services.service_graph_templates.name_suffix) }}
{% endif %}
...
Verify Contract {{ contract_name }} Subject {{ subject_name }} Consumer_to_Provider Filter {{ filter_name }}
    ...
    Should Be Equal Value Json String   ${r.json()}   ${filter}..vzRsFiltAtt.attributes.tnVzFilterName   {{ filter_name }}
    Should Be Equal Value Json String   ${r.json()}   ${filter}..vzRsFiltAtt.attributes.action   {{ filter.action | default(defaults.apic.tenants.contracts.subjects.consumer_to_provider.filters.action) }}
    Should Be Equal Value Json String   ${r.json()}   ${filter}..vzRsFiltAtt.attributes.directives   {{ directives | join(',') }}
    Should Be Equal Value Json String   ${r.json()}   ${filter}..vzRsFiltAtt.attributes.priorityOverride   {{ filter.priority | default(defaults.apic.tenants.contracts.subjects.consumer_to_provider.filters.priority) }}
{% endfor %}
{% endif %}
...
Verify Contract {{ contract_name }} Subject {{ subject_name }} Unidirectional Provider_to_Consumer
    ...
    Should Be Equal Value Json String   ${r.json()}   ${provider_to_consumer}..attributes.prio   {{ subject.provider_to_consumer.qos_class | default(defaults.apic.tenants.contracts.subjects.provider_to_consumer.qos_class) }}
    Should Be Equal Value Json String   ${r.json()}   ${provider_to_consumer}..attributes.targetDscp   {{ subject.provider_to_consumer.target_dscp | default(defaults.apic.tenants.contracts.subjects.provider_to_consumer.target_dscp) }}
 {% if subject.provider_to_consumer.service_graph is defined and subject.provider_to_consumer.filters is defined %}
    Should Be Equal Value Json String   ${r.json()}   ${provider_to_consumer}..vzRsOutTermGraphAtt.attributes.tnVnsAbsGraphName   {{ subject.provider_to_consumer.service_graph | default(defaults.apic.tenants.services.service_graph_templates.name_suffix) }}
{% endif %}
...
Verify Contract {{ contract_name }} Subject {{ subject_name }} Provider_to_Consumer Filter {{ filter_name }}
    ...
    Should Be Equal Value Json String   ${r.json()}   ${filter}..vzRsFiltAtt.attributes.tnVzFilterName   {{ filter_name }}
    Should Be Equal Value Json String   ${r.json()}   ${filter}..vzRsFiltAtt.attributes.action   {{ filter.action | default(defaults.apic.tenants.contracts.subjects.provider_to_consumer.filters.action) }}
    Should Be Equal Value Json String   ${r.json()}   ${filter}..vzRsFiltAtt.attributes.directives   {{ directives | join(',') }}
    Should Be Equal Value Json String   ${r.json()}   ${filter}..vzRsFiltAtt.attributes.priorityOverride   {{ filter.priority | default(defaults.apic.tenants.contracts.subjects.provider_to_consumer.filters.priority) }}
{% endfor %}
