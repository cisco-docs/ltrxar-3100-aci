...
Verify VRF {{ vrf_name }}
    ...
    Should Be Equal Value Json String   ${r}   $..fvCtx.attributes.name   {{ vrf_name }}
    Should Be Equal Value Json String   ${r}   $..fvCtx.attributes.nameAlias   {{ vrf.alias | default() }}
    Should Be Equal Value Json String   ${r}   $..fvCtx.attributes.descr   {{ vrf.description | default() }}
    Should Be Equal Value Json String   ${r}   $..fvCtx.attributes.ipDataPlaneLearning   {{ 'enabled' if vrf.data_plane_learning | default(defaults.apic.tenants.vrfs.data_plane_learning) else 'disabled' }}
{% if not vrf.ndo_managed | default(defaults.apic.tenants.vrfs.ndo_managed) %}
    Should Be Equal Value Json String   ${r}   $..fvCtx.attributes.pcEnfDir   {{ vrf.enforcement_direction | default(defaults.apic.tenants.vrfs.enforcement_direction) }}
{% endif %}
    Should Be Equal Value Json String   ${r}   $..fvCtx.attributes.pcEnfPref   {{ vrf.enforcement_preference | default(defaults.apic.tenants.vrfs.enforcement_preference) }}
    Should Be Equal Value Json String   ${r}   $..vzAny.attributes.prefGrMemb   {{ 'enabled' if vrf.preferred_group | default(defaults.apic.tenants.vrfs.preferred_group) else 'disabled' }}
...
{% set endpoint_retention_policy_name = vrf.endpoint_retention_policy ~ defaults.apic.tenants.policies.endpoint_retention_policies.name_suffix %}
    Should Be Equal Value Json String   ${r}   $..fvCtx.children..fvRsCtxToEpRet.attributes.tnFvEpRetPolName   {{ endpoint_retention_policy_name }}
{% endif %}
...
Verify VRF {{ vrf_name }} Leaked Internal Prefix {{ prefix.prefix }}
    ...
    Should Be Equal Value Json String   ${r}   ${prefix}.attributes.ip   {{ prefix.prefix }}
    Should Be Equal Value Json String   ${r}   ${prefix}.attributes.scope   {{ 'public' if prefix.public | default(defaults.apic.tenants.vrfs.leaked_internal_prefixes.public) else 'private' }}
...
Verify VRF {{ vrf_name }} Leaked Internal Prefix {{ prefix.prefix }} Destination {{ destination.tenant }} {{ vrf_name }}
    ...
    Should Be Equal Value Json String   ${r}   ${dest}.attributes.descr   {{ destination.description | default("") }}
    Should Be Equal Value Json String   ${r}   ${dest}.attributes.ctxName   {{ vrf_name }}
    Should Be Equal Value Json String   ${r}   ${dest}.attributes.tenantName   {{ destination.tenant }}
    Should Be Equal Value Json String   ${r}   ${dest}.attributes.scope   {{ ('public' if destination.public else 'private') if destination.public is defined else 'inherit' }}
...
Verify VRF {{ vrf_name }} Leaked External Prefix {{ prefix.prefix }}
    ...
    Should Be Equal Value Json String   ${r}   ${prefix}.attributes.ip   {{ prefix.prefix }}
    Should Be Equal Value Json String   ${r}   ${prefix}.attributes.le   {{ prefix.to_prefix_length | default('unspecified') }}
    Should Be Equal Value Json String   ${r}   ${prefix}.attributes.ge   {{ prefix.from_prefix_length | default('unspecified') }}
...
{% set transit_route_tag_policy_name = vrf.transit_route_tag_policy ~ defaults.apic.tenants.policies.route_tag_policies.name_suffix %}
    ...
    Should Be Equal Value Json String   ${r}   ${tag_entry}.attributes.tnL3extRouteTagPolName   {{ transit_route_tag_policy_name }}
{% endif %}
...
{% set bgp_timer_name = vrf.bgp.timer_policy + defaults.apic.tenants.policies.bgp_timer_policies.name_suffix %}
    ...
    Should Be Equal Value Json String   ${r}   ${bgp_entry}.attributes.tnBgpCtxPolName   {{ bgp_timer_name }}
{% endif %}
...
Verify IPv4 BGP Address Family Context Policy {{ vrf.bgp.ipv4_address_family_context_policy }}
    ...
    Should Be Equal Value Json String   ${r}   ${bgp_entry}.attributes.tnBgpCtxAfPolName   {{ address_family_context_policy_name }}
    Should Be Equal Value Json String   ${r}   ${bgp_entry}.attributes.af   ipv4-ucast
{% endif %}
...
Verify IPv6 BGP Address Family Context Policy {{ vrf.bgp.ipv6_address_family_context_policy }}
    ...
    Should Be Equal Value Json String   ${r}   ${bgp_entry}.attributes.tnBgpCtxAfPolName   {{ address_family_context_policy_name }}
    Should Be Equal Value Json String   ${r}   ${bgp_entry}.attributes.af   ipv6-ucast
{% endif %}
...
{% set address_family_context_policy_name = vrf.ospf.timer_policy ~ defaults.apic.tenants.policies.ospf_timer_policies.name_suffix %}
    ...
    Should Be Equal Value Json String   ${r}   ${ospf_entry}.attributes.tnOspfCtxPolName   {{ address_family_context_policy_name }}
{% endif %}
...
Verify OSPF Address Family IPv4 Context Policy {{ vrf.ospf.ipv4_address_family_context_policy }}
    ...
    Should Be Equal Value Json String   ${r}   ${ospf_entry}.attributes.tnOspfCtxPolName   {{ address_family_context_policy_name }}
    Should Be Equal Value Json String   ${r}   ${ospf_entry}.attributes.af   ipv4-ucast
{% endif %}
...
Verify OSPF Address Family IPv6 Context Policy {{ vrf.ospf.ipv6_address_family_context_policy }}
    ...
    Should Be Equal Value Json String   ${r}   ${ospf_entry}.attributes.tnOspfCtxPolName   {{ address_family_context_policy_name }}
    Should Be Equal Value Json String   ${r}   ${ospf_entry}.attributes.af   ipv6-ucast
{% endif %}
...
Verify BGP IPv4 Import Route Target {{ route_target }}
    ...
    Should Be Equal Value Json String   ${r}   ${bgp_entry}.attributes.rt    {{route_target}}
{% endfor %}
{% endif %}
...
Verify VRF {{ vrf.name }} vzAny Contract Provider {{ contract_name }}
    ...
    Should Be Equal Value Json String   ${r}   ${vzany_prov}.attributes.tnVzBrCPName   {{ contract_name }}
{% endfor %}
...
Verify VRF {{ vrf.name }} vzAny Contract consumers {{ contract_name }}
    ...
    Should Be Equal Value Json String   ${r}   ${vzany_cons}.attributes.tnVzBrCPName   {{ contract_name }}
...
Verify VRF {{ vrf.name }} vzAny Contract Import consumers {{ contract_name }}
    ...
    Should Be Equal Value Json String   ${r}   ${vzany_consIf}.attributes.tnVzCPIfName   {{ contract_name }}

{% endfor %}
{% if vrf.pim is defined %}

Verify VRF {{ vrf.name }} PIM
    Should Be Equal Value Json String   ${r}   $..fvCtx.children..pimCtxP.attributes.mtu   {{ vrf.pim.mtu | default(defaults.apic.tenants.vrfs.pim.mtu)}}
    Should Be Equal Value Json String   ${r}   $..fvCtx.children..pimCtxP.children..pimResPol.attributes.max   {{ max_entry(vrf.pim.max_multicast_entries | default(defaults.apic.tenants.vrfs.pim.max_multicast_entries)) }}
    Should Be Equal Value Json String   ${r}   $..fvCtx.children..pimCtxP.children..pimResPol.attributes.rsvd   {{ reserved_entry(vrf.pim.reserved_multicast_entries | default(defaults.apic.tenants.vrfs.pim.reserved_multicast_entries)) }}
...
{% set resource_policy_multicast_route_map_name = vrf.pim.resource_policy_multicast_route_map ~ defaults.apic.tenants.policies.multicast_route_maps.name_suffix %}
    Should Be Equal Value Json String   ${r}   $..fvCtx.children..pimCtxP.children..pimResPol.children..rtdmcRsFilterToRtMapPol.attributes.tDn   uni/tn-{{ tenant.name }}/rtmap-{{ resource_policy_multicast_route_map_name }}
{% endif %}
...
Verify VRF {{ vrf.name }} PIM BSR
    Should Be Equal Value Json String   ${r}   $..fvCtx.children..pimCtxP.children..pimBSRPPol.attributes.ctrl   {{ ctrl | join(',') }}
...
{% set bsr_filter_name = vrf.pim.bsr_filter_multicast_route_map ~ defaults.apic.tenants.policies.multicast_route_maps.name_suffix %}
    Should Be Equal Value Json String   ${r}   $..fvCtx.children..pimCtxP.children..pimBSRPPol.children..rtdmcRsFilterToRtMapPol.attributes.tDn   uni/tn-{{ tenant.name }}/rtmap-{{ bsr_filter_name }}
...
Verify VRF {{ vrf.name }} PIM Auto-RP
    Should Be Equal Value Json String   ${r}   $..fvCtx.children..pimCtxP.children..pimAutoRPPol.attributes.ctrl   {{ ctrl | join(',') }}
...
{% set auto_rp_filter_name = vrf.pim.auto_rp_filter_multicast_route_map ~ defaults.apic.tenants.policies.multicast_route_maps.name_suffix %}
    Should Be Equal Value Json String   ${r}   $..fvCtx.children..pimCtxP.children..pimAutoRPPol.children..rtdmcRsFilterToRtMapPol.attributes.tDn   uni/tn-{{ tenant.name }}/rtmap-{{ auto_rp_filter_name }}
...
Verify VRF {{ vrf.name }} PIM Static RP {{ static_rp.ip }}
    ...
    Should Be Equal Value Json String   ${r}   ${rp}.attributes.rpIp   {{ static_rp.ip }}
...
{% set static_rp_route_map_name = static_rp.multicast_route_map ~ defaults.apic.tenants.policies.multicast_route_maps.name_suffix %}
    Should Be Equal Value Json String   ${r}   ${rp}.children..pimRPGrpRangePol.children..rtdmcRsFilterToRtMapPol.attributes.tDn   uni/tn-{{ tenant.name }}/rtmap-{{ static_rp_route_map_name }}
...
Verify VRF {{ vrf.name }} PIM Fabric RP {{ fabric_rp.ip }}
    ...
    Should Be Equal Value Json String   ${r}   ${rp}.attributes.rpIp   {{ fabric_rp.ip }}
...
{% set fabric_rp_route_map_name = fabric_rp.multicast_route_map ~ defaults.apic.tenants.policies.multicast_route_maps.name_suffix %}
    Should Be Equal Value Json String   ${r}   ${rp}.children..pimRPGrpRangePol.children..rtdmcRsFilterToRtMapPol.attributes.tDn   uni/tn-{{ tenant.name }}/rtmap-{{ fabric_rp_route_map_name }}
...
{% set asm_shared_range_multicast_route_map_name = vrf.pim.asm_shared_range_multicast_route_map ~ defaults.apic.tenants.policies.multicast_route_maps.name_suffix %}
    Should Be Equal Value Json String   ${r}   $..fvCtx.children..pimCtxP.children..pimASMPatPol.children..pimSharedRangePol.children..rtdmcRsFilterToRtMapPol.attributes.tDn   uni/tn-{{ tenant.name }}/rtmap-{{ asm_shared_range_multicast_route_map_name }}
{% endif %}
...
{% set asm_sg_expiry_multicast_route_map_name = vrf.pim.asm_sg_expiry_multicast_route_map ~ defaults.apic.tenants.policies.multicast_route_maps.name_suffix %}
    Should Be Equal Value Json String   ${r}   $..fvCtx.children..pimCtxP.children..pimASMPatPol.children..pimSGRangeExpPol.children..rtdmcRsFilterToRtMapPol.attributes.tDn   uni/tn-{{ tenant.name }}/rtmap-{{ asm_sg_expiry_multicast_route_map_name }}
{% endif %}
    Should Be Equal Value Json String   ${r}   $..fvCtx.children..pimCtxP.children..pimASMPatPol.children..pimRegTrPol.attributes.maxRate   {{ vrf.pim.asm_traffic_registry_max_rate | default(defaults.apic.tenants.vrfs.pim.asm_traffic_registry_max_rate) }}
    Should Be Equal Value Json String   ${r}   $..fvCtx.children..pimCtxP.children..pimASMPatPol.children..pimRegTrPol.attributes.srcIp   {{ vrf.pim.asm_traffic_registry_source_ip | default(defaults.apic.tenants.vrfs.pim.asm_traffic_registry_source_ip) }}
...
{% set ssm_group_range_multicast_route_map_name = vrf.pim.ssm_group_range_multicast_route_map ~ defaults.apic.tenants.policies.multicast_route_maps.name_suffix %}
    Should Be Equal Value Json String   ${r}   $..fvCtx.children..pimCtxP.children..pimSSMPatPol.children..pimSSMRangePol.children..rtdmcRsFilterToRtMapPol.attributes.tDn   uni/tn-{{ tenant.name }}/rtmap-{{ ssm_group_range_multicast_route_map_name }}
{% endif %}
...
Verify VRF {{ vrf.name }} Inter-VRF Multicast Tenant {{ pol.tenant}} VRF {{ vrf_name }}
    ...
    Should Be Equal Value Json String   ${r}   ${inter_vrf}.attributes.srcVrfDn   uni/tn-{{ pol.tenant }}/ctx-{{ vrf_name }}
...
{% set multicast_route_map_name = pol.multicast_route_map ~ defaults.apic.tenants.policies.multicast_route_maps.name_suffix %}
    Should Be Equal Value Json String   ${r}   ${inter_vrf}.children..rtdmcRsFilterToRtMapPol.attributes.tDn   uni/tn-{{ tenant.name }}/rtmap-{{ multicast_route_map_name}}
{% endif %}
...
Verify VRF {{ vrf.name }} IGMP Context SSM Tranlation policies {{ pol.group_prefix }}-{{ pol.source_address }}
    ...
    Should Be Equal Value Json String   ${r}   ${igmp_ssn}.attributes.grpPfx   {{ pol.group_prefix }}
    Should Be Equal Value Json String   ${r}   ${igmp_ssn}.attributes.srcAddr   {{ pol.source_address }}
...
Verify VRF {{ vrf.name }} Route Summarization Policy {{ rsp_name }}
    ...
    Should Be Equal Value Json String   ${r}   ${pol}.attributes.name   {{ rsp_name }}
...
Verify VRF {{ vrf.name }} Route Summarization Policy {{ rsp_name }} Node {{ node.id }}
    ...
    Should Be Equal Value Json String   ${r}   ${node}.attributes.tDn   topology/pod-{{ pod }}/node-{{ node.id }}
...
Verify VRF {{ vrf.name }} Route Summarization Policy {{ rsp_name }} Subnet {{ subnet.prefix }}
    ...
    Should Be Equal Value Json String   ${r}   ${subnet}.attributes.prefix   {{ subnet.prefix }}
    Should Be Equal Value Json String   ${r}   ${subnet}.children..fvRsSubnetToRtSummPol.attributes.tDn   {{ brs_tdn }}
