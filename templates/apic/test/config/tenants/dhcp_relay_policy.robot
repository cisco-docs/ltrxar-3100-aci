...
Verify DHCP Relay Policy {{ policy_name }}
    ...
    Should Be Equal Value Json String   ${r}   $..dhcpRelayP.attributes.name   {{ policy_name }}
    Should Be Equal Value Json String   ${r}   $..dhcpRelayP.attributes.descr  {{ policy.description | default() }}
...
Verify DHCP Relay Policy {{ policy_name }} Provider {{ provider.ip }}
    ...
    Should Be Equal Value Json String   ${r}   ${provider}..dhcpRsProv.attributes.addr   {{ provider.ip }}
...
{% set epg_name = provider.endpoint_group ~ defaults.apic.tenants.application_profiles.endpoint_groups.name_suffix %}
    Should Be Equal Value Json String   ${r}   ${provider}..dhcpRsProv.attributes.tDn   uni/tn-{{ provider.tenant | default(tenant.name) }}/ap-{{ ap_name }}/epg-{{ epg_name }}
...
{% set eepg_name = provider.external_endpoint_group ~ defaults.apic.tenants.l3outs.external_endpoint_groups.name_suffix %}
    Should Be Equal Value Json String   ${r}   ${provider}..dhcpRsProv.attributes.tDn   uni/tn-{{ provider.tenant | default(tenant.name) }}/out-{{ l3out_name }}/instP-{{ eepg_name }}
{% endif %}
