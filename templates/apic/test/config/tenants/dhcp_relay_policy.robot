...
Verify DHCP Relay Policy {{ policy_name }}
    ...
    Should Be Equal JMESPath Json   ${r}   imdata[0].dhcpRelayP.attributes.name   {{ policy_name }}
    Should Be Equal JMESPath Json   ${r}   imdata[0].dhcpRelayP.attributes.descr  {{ policy.description | default() }}

{% for provider in policy.providers | default([]) %}

Verify DHCP Relay Policy {{ policy_name }} Provider {{ provider.ip }}
    ${provider}=   Set Variable   imdata[0].dhcpRelayP.children[?dhcpRsProv.attributes.addr=='{{ provider.ip }}'] | [0]
    Should Be Equal JMESPath Json   ${r}   ${provider}.dhcpRsProv.attributes.addr   {{ provider.ip }}
...
{% set epg_name = provider.endpoint_group ~ defaults.apic.tenants.application_profiles.endpoint_groups.name_suffix %}
    Should Be Equal JMESPath Json   ${r}   ${provider}.dhcpRsProv.attributes.tDn   uni/tn-{{ provider.tenant | default(tenant.name) }}/ap-{{ ap_name }}/epg-{{ epg_name }}
...
{% set eepg_name = provider.external_endpoint_group ~ defaults.apic.tenants.l3outs.external_endpoint_groups.name_suffix %}
    Should Be Equal JMESPath Json   ${r}   ${provider}.dhcpRsProv.attributes.tDn   uni/tn-{{ provider.tenant | default(tenant.name) }}/out-{{ l3out_name }}/instP-{{ eepg_name }}
{% endif %}
