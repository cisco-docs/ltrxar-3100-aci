...
{% endif %}
    ...
    Should Be Equal Value Json String   ${r}   $..fvRsScope.attributes.tnFvCtxName   {{ esg.vrf ~ ('' if esg.vrf in ('inb', 'obb', 'overlay-1') else defaults.apic.tenants.vrfs.name_suffix) }}
...
Verify Endpoint Security Group {{ esg_name }} Contract Master Tenant {{ tenant.name }} AP {{ ap_name }} ESG {{ esg_name }}
    ...
    Should Be Equal Value Json String   ${r}   ${con}..fvRsSecInherited.attributes.tDn   uni/tn-{{ tenant.name }}/ap-{{ ap_name }}/esg-{{ esg_name }}
...
Verify Endpoint Security Group {{ esg_name }} Tag Selector Key {{ sel.key }} Value {{ sel.value }}
    ...
    Should Be Equal Value Json String   ${r}   ${con}..fvTagSelector.attributes.descr   {{ sel.description | default() }}
    Should Be Equal Value Json String   ${r}   ${con}..fvTagSelector.attributes.matchKey   {{ sel.key }}
    Should Be Equal Value Json String   ${r}   ${con}..fvTagSelector.attributes.matchValue   {{ sel.value }}
    Should Be Equal Value Json String   ${r}   ${con}..fvTagSelector.attributes.valueOperator   {{ sel.operator | default(defaults.apic.tenants.application_profiles.endpoint_security_groups.tag_selectors.operator) }}
...
Verify Endpoint Security Group {{ esg_name }} EPG Selector Tenant {{ master.tenant | default(tenant.name) }} AP {{ ap_name }} EPG {{ epg_name }}
    ...
    Should Be Equal Value Json String   ${r}   ${con}..fvEPgSelector.attributes.descr   {{ sel.description | default() }}
    Should Be Equal Value Json String   ${r}   ${con}..fvEPgSelector.attributes.matchEpgDn   uni/tn-{{ master.tenant | default(tenant.name) }}/ap-{{ ap_name }}/epg-{{ epg_name }}
...
Verify Endpoint Security Group {{ esg_name }} IP Subnet Selector {{ sel.value }}
    ...
    Should Be Equal Value Json String   ${r}   ${con}..fvEPSelector.attributes.descr   {{ sel.description | default() }}
    Should Be Equal Value Json String   ${r}   ${con}..fvEPSelector.attributes.matchExpression   ip=='{{ sel.value }}'
