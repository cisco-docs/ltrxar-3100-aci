...
{% set ctrl = [] %}
{% if eip.bfd | default(defaults.apic.tenants.policies.eigrp_interface_policies.bfd) %}{% set ctrl = ctrl + [("bfd")] %}{% endif %}
{% if eip.self_nexthop | default(defaults.apic.tenants.policies.eigrp_interface_policies.self_nexthop) %}{% set ctrl = ctrl + [("nh-self")] %}{% endif %}
{% if eip.passive_interface | default(defaults.apic.tenants.policies.eigrp_interface_policies.passive_interface) %}{% set ctrl = ctrl + [("passive")] %}{% endif %}
{% if eip.split_horizon | default(defaults.apic.tenants.policies.eigrp_interface_policies.split_horizon) %}{% set ctrl = ctrl + [("split-horizon")] %}{% endif %}
