...
Verify Monitoring Policy {{ policy_name }}
    ...
    Should Be Equal Value Json String   ${r.json()}    $..monEPGPol.attributes.name   {{ policy_name }}
...
{% if snmp.destination_group is defined %}
    Should Be Equal Value Json String   ${r.json()}    ${mon}..snmpSrc.children..snmpRsDestGroup.attributes.tDn   uni/fabric/snmpgroup-{{ snmp.destination_group ~ defaults.apic.fabric_policies.monitoring.snmp_traps.name_suffix }}
{% endif %}
{% endfor %}
...
Verify Monitoring Policy {{ policy_name }} Syslog Policy {{ syslog_policy_name }}
    ...
    Should Be Equal Value Json String   ${r.json()}    ${sysl}..syslogSrc.attributes.minSev   {{ syslog.minimum_severity | default(defaults.apic.tenants.policies.monitoring.policies.syslogs.minimum_severity) }}
{% if syslog.destination_group is defined %}
    Should Be Equal Value Json String   ${r.json()}    ${sysl}..syslogSrc.children..syslogRsDestGroup.attributes.tDn   uni/fabric/slgroup-{{ syslog.destination_group ~ defaults.apic.fabric_policies.monitoring.syslogs.name_suffix }}
{% endif %}
{% endfor %}
...
Verify Monitoring Policy {{ policy_name }} Fault Severity Policy Class {{ cl.class }}
    Should Be Equal Value Json String   ${r.json()}    $..monEPGTarget.attributes.scope  {{ cl.class }}
...
Verify Monitoring Policy {{ policy_name }} Fault Severity Policy Class {{ cl.class }} Fault {{ fault.fault_id }}
    ...
    Should Be Equal Value Json String   ${r.json()}    ${sev}..faultSevAsnP.attributes.initial  {{ fault.initial_severity | default(defaults.apic.tenants.policies.monitoring.policies.fault_severity_policies.initial_severity) }}
    Should Be Equal Value Json String   ${r.json()}    ${sev}..faultSevAsnP.attributes.target  {{ fault.target_severity | default(defaults.apic.tenants.policies.monitoring.policies.fault_severity_policies.target_severity) }}
