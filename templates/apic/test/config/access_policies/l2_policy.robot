...
Verify L2 Interface Policy {{ l2_policy_name }}
    ...
    Should Be Equal Value Json String   ${r.json()}    $..l2IfPol.attributes.vepa   {{ 'enabled' if policy.reflective_relay | default(defaults.apic.access_policies.interface_policies.l2_policies.reflective_relay) == true else 'disabled' }}
