...
{% if pg.type == "breakout" %}
    ...
    Should Be Equal Value Json String   ${r}    $..infraBrkoutPortGrp.attributes.name   {{ policy_group_name }}
    Should Be Equal Value Json String   ${r}    $..infraBrkoutPortGrp.attributes.descr   {{ pg.description | default() }}
    Should Be Equal Value Json String   ${r}    $..infraBrkoutPortGrp.attributes.brkoutMap   {{ pg.map | default(defaults.apic.access_policies.leaf_interface_policy_groups.map)}}
{% elif pg.type in ["vpc", "pc"] %}
    ...
    Set Suite Variable   $r   ${r.json()}
    Should Be Equal Value Json String   ${r}    $..infraAccBndlGrp.attributes.name   {{ policy_group_name }}
    Should Be Equal Value Json String   ${r}    $..infraAccBndlGrp.attributes.descr   {{ pg.description | default() }}
{% if pg.type == "vpc" %}
    Should Be Equal Value Json String   ${r}    $..infraAccBndlGrp.attributes.lagT   node
{% elif pg.type == "pc" %}
    Should Be Equal Value Json String   ${r}    $..infraAccBndlGrp.attributes.lagT   link
{% endif %}
{% else %}
    ${r}=   GET On Session   apic   /api/mo/uni/infra/funcprof/accportgrp-{{ policy_group_name }}.json   params=rsp-subtree=full
    Set Suite Variable   $r   ${r.json()}
    Should Be Equal Value Json String   ${r}    $..infraAccPortGrp.attributes.name   {{ policy_group_name }}
    Should Be Equal Value Json String   ${r}    $..infraAccPortGrp.attributes.descr   {{ pg.description | default() }}
{% endif %}
...
{% set lldp_policy_name = pg.lldp_policy ~ defaults.apic.access_policies.interface_policies.lldp_policies.name_suffix %}
    Should Be Equal Value Json String   ${r}    $..infraRsLldpIfPol.attributes.tnLldpIfPolName   {{ lldp_policy_name }}
{% endif %}
...
{% set spanning_tree_policy_name = pg.spanning_tree_policy ~ defaults.apic.access_policies.interface_policies.spanning_tree_policies.name_suffix %}
    Should Be Equal Value Json String   ${r}    $..infraRsStpIfPol.attributes.tnStpIfPolName   {{ spanning_tree_policy_name }}
{% endif %}
...
{% set mcp_policy_name = pg.mcp_policy ~ defaults.apic.access_policies.interface_policies.mcp_policies.name_suffix %}
    Should Be Equal Value Json String   ${r}    $..infraRsMcpIfPol.attributes.tnMcpIfPolName   {{ mcp_policy_name }}
{% endif %}
...
{% set l2_policy_name = pg.l2_policy ~ defaults.apic.access_policies.interface_policies.l2_policies.name_suffix %}
    Should Be Equal Value Json String   ${r}    $..infraRsL2IfPol.attributes.tnL2IfPolName   {{ l2_policy_name }}
{% endif %}
...
{% set storm_control_policy_name = pg.storm_control_policy ~ defaults.apic.access_policies.interface_policies.storm_control_policies.name_suffix %}
    Should Be Equal Value Json String   ${r}    $..infraRsStormctrlIfPol.attributes.tnStormctrlIfPolName   {{ storm_control_policy_name }}
{% endif %}
...
{% set port_channel_policy_name = pg.port_channel_policy ~ defaults.apic.access_policies.interface_policies.port_channel_policies.name_suffix %}
    Should Be Equal Value Json String   ${r}    $..infraRsLacpPol.attributes.tnLacpLagPolName   {{ port_channel_policy_name }}
{% endif %}
...
{% set port_channel_member_policy_name = pg.port_channel_member_policy ~ defaults.apic.access_policies.interface_policies.port_channel_member_policies.name_suffix %}
    Should Be Equal Value Json String   ${r}    $..infraAccBndlSubgrp.attributes.name   {{ policy_group_name }}
    Should Be Equal Value Json String   ${r}    $..infraRsLacpInterfacePol.attributes.tnLacpIfPolName   {{ port_channel_member_policy_name }}
{% endif %}
...
{% endif %}
    ...
    Should Be Equal Value Json String   ${r}    ${mon}.attributes.fltType   {{ monitor.ip_filter_type | default(defaults.apic.access_policies.leaf_interface_policy_groups.netflow_monitor_policies.ip_filter_type) }}
{% endfor %}
...
{% if pg.ingress_data_plane_policing_policy is defined %}
    ...
    Should Be Equal Value Json String   ${r}   $..infraRsQosIngressDppIfPol.attributes.tnQosDppPolName   {{ dpp_name }}
{% endif %}
{% if pg.egress_data_plane_policing_policy is defined %}
    ...
    Should Be Equal Value Json String   ${r}   $..infraRsQosEgressDppIfPol.attributes.tnQosDppPolName   {{ dpp_name }}
{% endif %}
