...
Verify MST Switch Policy {{ mst_policy_name }}
    ...
    Should Be Equal Value Json String   ${r}    $..stpMstRegionPol.attributes.name   {{ mst_policy_name }}
...
Verify MST Switch Policy {{ mst_policy_name }} Instance {{ inst.name }}
    ...
    Should Be Equal Value Json String   ${r}    ${inst}..stpMstDomPol.attributes.name   {{ inst.name }}
    Should Be Equal Value Json String   ${r}    ${inst}..stpMstDomPol.attributes.id   {{ inst.id }}
