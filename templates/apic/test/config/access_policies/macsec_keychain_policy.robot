...
{% if key_policy.start_time | default(defaults.apic.access_policies.interface_policies.macsec_keychain_policies.key_policies.start_time) != "now" %}
    Should Be Equal Value Json String   ${r}    ${key}.attributes.startTime   {{ key_policy.start_time | default(defaults.apic.access_policies.interface_policies.macsec_keychain_policies.key_policies.start_time) }}
{% endif %}
    Should Be Equal Value Json String   ${r}    ${key}.attributes.endTime   {{ key_policy.end_time | default(defaults.apic.access_policies.interface_policies.macsec_keychain_policies.key_policies.end_time) }}
