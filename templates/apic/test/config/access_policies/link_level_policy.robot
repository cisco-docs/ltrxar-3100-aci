...
{% if policy.link_delay_interval is defined %}
    Should Be Equal Value Json String   ${r.json()}    $..fabricHIfPol.attributes.dfeDelayMs   {{ policy.link_delay_interval }}
{% endif %}
    Should Be Equal Value Json String   ${r.json()}    $..fabricHIfPol.attributes.linkDebounce   {{ policy.link_debounce_interval | default(defaults.apic.access_policies.interface_policies.link_level_policies.link_debounce_interval) }}
