...
{% set epg_name = provider.endpoint_group ~ defaults.apic.tenants.application_profiles.endpoint_groups.name_suffix %}
    Should Be Equal Value Json String   ${r}   ${provider}..dhcpRsProv.attributes.tDn   uni/tn-{{ provider.tenant }}/ap-{{ ap_name }}/epg-{{ epg_name }}
...
{% set eepg_name = provider.external_endpoint_group ~ defaults.apic.tenants.l3outs.external_endpoint_groups.name_suffix %}
    Should Be Equal Value Json String   ${r}   ${provider}..dhcpRsProv.attributes.tDn   uni/tn-{{ provider.tenant }}/out-{{ l3out_name }}/instP-{{ eepg_name }}
{% endif %}
