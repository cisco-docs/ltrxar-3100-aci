...
{% if snmp.destination_group is defined %}
    Should Be Equal Value Json String   ${r}    ${mon}..snmpSrc.children..snmpRsDestGroup.attributes.tDn   uni/fabric/snmpgroup-{{ snmp.destination_group ~ defaults.apic.access_policies.monitoring.policies.snmp_traps.name_suffix }}
{% endif %}
{% endfor %}
...
Verify Monitoring Policy {{ policy_name }} Syslog Policy {{ syslog_policy_name }}
    ...
    Should Be Equal Value Json String   ${r}    ${sysl}..syslogSrc.attributes.minSev   {{ syslog.minimum_severity | default(defaults.apic.access_policies.monitoring.policies.syslogs.minimum_severity) }}
{% if syslog.destination_group is defined %}
    Should Be Equal Value Json String   ${r}    ${sysl}..syslogSrc.children..syslogRsDestGroup.attributes.tDn   uni/fabric/slgroup-{{ syslog.destination_group ~ defaults.apic.access_policies.monitoring.policies.syslogs.name_suffix }}
{% endif %}
{% endfor %}
...
Verify Monitoring Policy {{ policy_name }} Fault Severity Policy Class {{ cl.class }} Fault {{ fault.fault_id }}
    ${sev}=   Set Variable    $..monInfraTarget.children[?(@.faultSevAsnP.attributes.code=='{{ fault.fault_id }}')]
    Should Be Equal Value Json String   ${r}    ${sev}..faultSevAsnP.attributes.code  {{ fault.fault_id }}
    Should Be Equal Value Json String   ${r}    ${sev}..faultSevAsnP.attributes.initial  {{ fault.initial_severity | default(defaults.apic.access_policies.monitoring.policies.fault_severity_policies.initial_severity) }}
    Should Be Equal Value Json String   ${r}    ${sev}..faultSevAsnP.attributes.target  {{ fault.target_severity | default(defaults.apic.access_policies.monitoring.policies.fault_severity_policies.target_severity) }}
