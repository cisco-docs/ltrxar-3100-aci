...
Verify MCP Global Instance
    ...
    Should Be Equal Value Json String   ${r.json()}    $..mcpInstPol.attributes.ctrl   {{ 'pdu-per-vlan' if apic.access_policies.mcp.per_vlan | default(defaults.apic.access_policies.mcp.per_vlan) else '' }}
    ...
    Should Be Equal Value Json String   ${r.json()}    $..mcpInstPol.attributes.loopProtectAct   {{ 'port-disable' if apic.access_policies.mcp.action | default(defaults.apic.access_policies.mcp.action) else '' }}
