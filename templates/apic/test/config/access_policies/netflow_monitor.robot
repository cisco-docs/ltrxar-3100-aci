...
Verify Netflow Monitor {{ monitor_name }}
    ...
    Should Be Equal Value Json String   ${r}    $..netflowMonitorPol.attributes.name   {{ monitor_name }}
    Should Be Equal Value Json String   ${r}    $..netflowMonitorPol.attributes.descr   {{ monitor.description | default() }}
...
Verify Netflow Monitor {{ monitor_name }} Record {{ record_name }}
    Should Be Equal Value Json String   ${r}    $..netflowMonitorPol.children..tnNetflowRecordPolName   {{ record_name }}
{% endif %}
...
Verify Netflow Monitor {{ monitor_name }} Exporter {{ exporter_name }}
    ...
    Should Be Equal Value Json String   ${r}    ${exp}.attributes.tnNetflowExporterPolName   {{ exporter_name }}
{% endfor %}
