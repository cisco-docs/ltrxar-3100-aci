...
{% if span.node_id is defined or span.channel is defined %}
    Should Be Equal JMESPath Json   ${r}    imdata[0].spanDestGrp.children[?spanDest] | [0].spanDest.children[?spanRsDestPathEp] | [0].spanRsDestPathEp.attributes.mtu   {{ span.mtu | default(defaults.apic.access_policies.span.destination_groups.mtu) }}
{% if span.node_id is defined and span.channel is not defined %}
    ...
    Should Be Equal JMESPath Json   ${r}    imdata[0].spanDestGrp.children[?spanDest] | [0].spanDest.children[?spanRsDestPathEp] | [0].spanRsDestPathEp.attributes.tDn   topology/pod-{{ pod }}/paths-{{ span.node_id }}/pathep-[eth{{ span.module | default(defaults.apic.access_policies.span.destination_groups.module) }}/{{ span.port }}/{{ span.sub_port }}]
    {%else%}
    Should Be Equal JMESPath Json   ${r}    imdata[0].spanDestGrp.children[?spanDest] | [0].spanDest.children[?spanRsDestPathEp] | [0].spanRsDestPathEp.attributes.tDn   topology/pod-{{ pod }}/paths-{{ span.node_id }}/pathep-[eth{{ span.module | default(defaults.apic.access_policies.span.destination_groups.module) }}/{{ span.port }}]
    {% endif %}
{% else %}
    ...
    {% if id_sub_ports | length > 0 %}
        ...
        {% set pod = span.pod_id | default(((apic.node_policies | default()) | community.general.json_query(query))[0] | default('1')) %}
    Should Be Equal JMESPath Json   ${r}    imdata[0].spanDestGrp.children[?spanDest] | [0].spanDest.children[?spanRsDestPathEp] | [0].spanRsDestPathEp.attributes.tDn   topology/pod-{{ pod }}/paths-{{ node }}/pathep-[{{ policy_group_name }}]
...
{% set endpoint_group_name = span.endpoint_group ~ defaults.apic.tenants.application_profiles.endpoint_groups.name_suffix %}
    ...
    Should Be Equal JMESPath Json   ${r}    imdata[0].spanDestGrp.children[?spanDest] | [0].spanDest.children[?spanRsDestEpg] | [0].spanRsDestEpg.attributes.dscp   {{ span.dscp | default(defaults.apic.access_policies.span.destination_groups.dscp) }}
    Should Be Equal JMESPath Json   ${r}    imdata[0].spanDestGrp.children[?spanDest] | [0].spanDest.children[?spanRsDestEpg] | [0].spanRsDestEpg.attributes.flowId   {{ span.flow_id | default(defaults.apic.access_policies.span.destination_groups.flow_id) }}
    Should Be Equal JMESPath Json   ${r}    imdata[0].spanDestGrp.children[?spanDest] | [0].spanDest.children[?spanRsDestEpg] | [0].spanRsDestEpg.attributes.mtu   {{ span.mtu | default(defaults.apic.access_policies.span.destination_groups.mtu) }}
    Should Be Equal JMESPath Json   ${r}    imdata[0].spanDestGrp.children[?spanDest] | [0].spanDest.children[?spanRsDestEpg] | [0].spanRsDestEpg.attributes.ttl   {{ span.ttl | default(defaults.apic.access_policies.span.destination_groups.ttl) }}
    Should Be Equal JMESPath Json   ${r}    imdata[0].spanDestGrp.children[?spanDest] | [0].spanDest.children[?spanRsDestEpg] | [0].spanRsDestEpg.attributes.ver   ver{{ span.version | default(defaults.apic.access_policies.span.destination_groups.version) }}
    Should Be Equal JMESPath Json   ${r}    imdata[0].spanDestGrp.children[?spanDest] | [0].spanDest.children[?spanRsDestEpg] | [0].spanRsDestEpg.attributes.verEnforced   {{ 'yes' if span.enforce_version | default(defaults.apic.access_policies.span.destination_groups.enforce_version) else 'no' }}
