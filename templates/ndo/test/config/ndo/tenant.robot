...
Verify Tenant {{ tenant.name }}
    ...
    Should Be Equal JMESPath Json   ${r.json()}   ${tenant}.name   {{ tenant.name }}
    Should Be Equal JMESPath Json   ${r.json()}   ${tenant}.displayName   {{ tenant.name }}
    Should Be Equal JMESPath Json   ${r.json()}   ${tenant}.description   {{ tenant.description | default() }}
...
Verify Tenant {{ tenant.name }} Site {{ site.name }}
    ...
    Should Be Equal JMESPath Json   ${r.json()}   ${site}.siteId   ${site_id}
{% if site.azure_subscription_id is defined %}
    Should Be Equal JMESPath Json   ${r.json()}   ${site}.cloudAccount   uni/tn-{{ site.azure_shared_tenant | default(tenant.name) }}/act-[{{ site.azure_subscription_id }}]-vendor-azure
{% if site.azure_shared_tenant is not defined %}
    Should Be Equal JMESPath Json   ${r.json()}   ${site}.cloudSubscriptionId   {{ site.azure_subscription_id }}
{% endif %}
{% endif %}
...
Verify Tenant {{ tenant.name }} User {{ user.name }}
    ...
    Should Be Equal JMESPath Json   ${r.json()}   ${user}.userId   ${user_id}
