...
Verify DHCP Relay Policy {{ pol_name }}
    ${pol}=   Set Variable   DhcpRelayPolicies[?name=='{{ pol_name }}'] | [0]
    Should Be Equal JMESPath Json   ${r.json()}   ${pol}.name   {{ pol_name }}
    Should Be Equal JMESPath Json   ${r.json()}   ${pol}.desc   {{ pol.description | default() }}
    ${tenant_id}=   NDO Lookup   tenants   {{ pol.tenant }}
    Should Be Equal JMESPath Json   ${r.json()}   ${pol}.tenantId   ${tenant_id}

{% for prov in pol.providers | default([]) %}

Verify DHCP Relay Policy {{ pol_name }} Provider {{ prov.ip }}
    ${prov}=   Set Variable   DhcpRelayPolicies[?name=='{{ pol_name }}'].provider[?addr=='{{ prov.ip }}'] | [0]
    Should Be Equal JMESPath Json   ${r.json()}   ${prov}.addr   {{ prov.ip }}
...
{% set epg_name = prov.endpoint_group ~ defaults.ndo.schemas.templates.application_profiles.endpoint_groups.name_suffix %}
    Should Be Equal JMESPath Json   ${r.json()}   ${prov}.epgRef   /schemas/${schema_id}/templates/{{ prov.template }}/anps/{{ ap_name }}/epgs/{{ epg_name }}
{% else %}
{% set ext_epg_name = prov.external_endpoint_group ~ defaults.ndo.schemas.templates.external_endpoint_groups.name_suffix %}
    Should Be Equal JMESPath Json   ${r.json()}   ${prov}.externalEpgRef   /schemas/${schema_id}/templates/{{ prov.template }}/externalEpgs/{{ ext_epg_name }}
{% endif %}
    ${tenant_id}=   NDO Lookup   tenants   {{ prov.tenant }}
    Should Be Equal JMESPath Json   ${r.json()}   ${prov}.tenantId   ${tenant_id}
