                ...
                if ptp.get("template", "aes67") == "aes67":
                    ...
                    if not -4 <= ptp.get("sync_interval", -3) <= 1:
                        results.append(
                            ...
                            + ptp.get("name")
                            + " is using AES67 where sync interval is not in the range of -4 to 1."
                        )
                    ...
                    if not -3 <= ptp.get("announce_interval", 1) <= 1:
                        results.append(
                            "apic.access_policies.ptp_profiles.announce_interval - "
                            + ptp.get("name")
                            + " is using SMPTE where announce interval is not in the range of -3 to 1."
                        )
                    ...
                    if not -4 <= ptp.get("sync_interval", -3) <= -1:
                        results.append(
                            "apic.access_policies.ptp_profiles.sync_interval - "
                            + ptp.get("name")
                            + " is using SMPTE where sync interval is not in the range of -4 to -1."
                        )
                    ...
                    if not -4 <= ptp.get("delay_interval", -3) <= 4:
                        results.append(
                            "apic.access_policies.ptp_profiles.delay_interval - "
                            + ptp.get("name")
                            + " is using SMPTE where delay interval is not in the range of -4 to 4."
                        )
                    ...
                    if ptp.get("announce_interval", 1) != -3:
                        results.append(
                            "apic.access_policies.ptp_profiles.announce_interval - "
                            + ptp.get("name")
                            + " is using Telecom G.8275.1 where announce interval can only be -3."
                        )
                    ...
                    if ptp.get("delay_interval", -3) != -4:
                        results.append(
                            "apic.access_policies.ptp_profiles.delay_interval - "
                            + ptp.get("name")
                            + " is using Telecom G.8275.1 where delay interval can only be -4."
                        )

                    if ptp.get("sync_interval", -4) != -4:
                        results.append(
                            "apic.access_policies.ptp_profiles.sync_interval - "
                            + ptp.get("name")
                            + " is using Telecom G.8275.1 where sync interval can only be -4."
                        )
